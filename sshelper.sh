...
#
# Author: Gemini
# Description: A comprehensive, menu-driven script to manage Fail2Ban and
#              harden SSH security on Debian systems.
#              - Now supports GitHub username input for public key retrieval.
# ==============================================================================
...
manage_fail2ban() {
    ...
    while true; do show_fail2ban_menu; read -p "选择: " choice
        case "$choice" in
            1) show_status; pause ;;
            2) manage_unban ;;
            3) show_log; pause ;;
...
manage_unban() { while true; do clear; echo -e "${CYAN}--- 解封管理 ---\n  1. 手动输入IP\n  2. 列表选择\n  3. 全部解封\n\n  ${YELLOW}b. 返回${NC}\n${CYAN}----------------${NC}"; read -p "选择: " choice; case "$choice" in 1) unban_by_ip; pause ;; 2) unban_by_number; pause ;; 3) unban_all_sshd; pause ;; b|B) break ;; *) error "无效输入"; sleep 1 ;; esac; done; }
unban_by_ip() { read -p "请输入IP: " ip; if [ -z "$ip" ]; then error "IP不能为空"; return; fi; info "尝试从[sshd]解封IP: ${ip}..."; local out; out=$(fail2ban-client set sshd unbanip "$ip"); if [ "$out" = "1" ]; then info "✅ 成功解封"; else warn "IP未在[sshd]封禁列表或解封失败"; fi; }
unban_by_number() { info "获取IP列表..."; local ip_str; ip_str=$(fail2ban-client status sshd | grep 'Banned IP list:' | sed 's/.*Banned IP list:\s*//'); if [ -z "$ip_str" ]; then info "当前无被封禁IP"; return; fi; local -a ips; read -r -a ips <<< "$ip_str"; echo "当前被封禁IP:"; for i in "${!ips[@]}"; do echo -e "  ${YELLOW}$((i+1)).${NC} ${ips[$i]}"; done; read -p "输入编号: " num; if ! [[ "$num" =~ ^[0-9]+$ ]] || [ "$num" -lt 1 ] || [ "$num" -gt ${#ips[@]} ]; then error "无效编号"; return; fi; local ip=${ips[$((num-1))]}; info "解封IP: ${ip}..."; local out; out=$(fail2ban-client set sshd unbanip "$ip"); if [ "$out" = "1" ]; then info "✅ 成功解封"; else warn "解封失败"; fi; }
unban_all_sshd() { warn "确定解封SSHD jail中所有IP?"; read -p "(y/N): " choice; case "$choice" in y|Y) info "执行解封所有IP..."; local ip_str; ip_str=$(fail2ban-client status sshd | grep 'Banned IP list:' | sed 's/.*Banned IP list:\s*//'); if [ -z "$ip_str" ]; then info "无IP需解封"; return; fi; local -a ips; read -r -a ips <<< "$ip_str"; local count=0; for ip in "${ips[@]}"; do if [ "$(fail2ban-client set sshd unbanip "$ip")" = "1" ]; then ((count++)); fi; done; info "✅ 操作完成, 共解封${count}个IP";; *) info "操作取消";; esac; }
...
manage_ssh() {
    while true; do show_ssh_menu; read -p "选择: " choice
        case "$choice" in
            1) view_ssh_config; pause ;;
            2) change_ssh_port; pause ;;
            3) toggle_password_auth; pause ;;
            4 | 4) add_key_from_input; pause ;; # Renamed function for clarity
...
change_ssh_port() {
    ...
    read -p "输入新端口号(1025-65535): " new_port
...
change_user_password() { read -p "输入用户名(默认root): " user; user=${user:-root}; info "为用户'${user}'修改密码..."; passwd "$user"; }
add_key_from_input() { # Renamed and updated function
    read -p "请输入要添加密钥的用户名 (默认root): " username
    username=${username:-root}; if ! id "$username" &>/dev/null; then error "用户'${username}'不存在"; return; fi

    read -p "请输入 GitHub 用户名 或 完整的公钥URL: " user_input
    if [ -z "$user_input" ]; then error "输入不能为空。"; return; fi

    local key_url
    # --- NEW: Smartly determine the URL ---
    ...
    # Use grep's file-based comparison to handle multiple keys from GitHub
...
toggle_password_auth() {
    ...
    local new_status; if [ "$status" = "yes" ]; then
        warn "准备关闭密码登录!"; warn "关闭前请务必确认密钥登录可用, 否则将无法登录!"; read -p "确定关闭? (y/N): " choice; if [[ "$choice" != "y" && "$choice" != "Y" ]]; then info "操作取消"; return; fi; new_status="no"
    else read -p "确定开启密码登录? (y/N): " choice; if [[ "$choice" != "y" && "$choice" != "Y" ]]; then info "操作取消"; return; fi; new_status="yes"; fi
...
restart_sshd() { warn "重启SSHD可能中断当前连接!"; read -p "确定? (y/N): " choice; case "$choice" in y|Y) info "重启SSHD..."; systemctl restart sshd; sleep 1; if systemctl is-active --quiet sshd; then info "✅ SSHD运行中"; else error "❌ SSHD重启失败!"; fi;; *) info "操作取消";; esac; }
...
manage_installation() {
    while true; do show_installation_menu; read -p "选择: " choice
        case "$choice" in
            1) do_install; pause ;;
